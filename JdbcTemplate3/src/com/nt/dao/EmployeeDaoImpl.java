...
public class EmployeeDaoImpl implements EmployeeDao {
	...
	public String addEmployee(Employee employee) {
		String status = null;
		List<Employee> list = jdbcTemplate.query("select * from employee where eid=?" ,new Object[] {employee.getEid()} ,
		...
		if (list.isEmpty() == true) {
			int rowcount = jdbcTemplate.update("insert into employee values(?,?,?,?)",new Object[] {employee.getEid(),employee.getEname(),employee.getEsal(),employee.getEaddr()});
			if (rowcount == 1) {
	...
	public Employee searchEmployee(int eid) {
		...
		List<Employee> list = jdbcTemplate.query("select * from employee where eid=?",new Object[] {eid}, new RowMaperDao());
		if (list.isEmpty() == true) {
			employee = null;
		} else {
			employee = list.get(0);
	...
	//List<Employee> employee1=jdbcTemplate.query("select * from employee where eid=?",new Object[] {employee.getEid()}, new RowMaperDao());
		...
		int rowCount=jdbcTemplate.update("update employee set ename=?,esal=?,eaddr=? where eid=?",new Object[] {employee.getEname(),employee.getEsal(),employee.getEaddr(),employee.getEid()});
	if (rowCount==1) {
	...
	public String deleteEmployee(int eid) {
		...
		if (employee == null) {
			...
			int rowcount = jdbcTemplate.update("delete from employee where eid=?",new Object[] {eid});
			if (rowcount == 1) {
